...
import type { AgentConfig } from './domain/models/config.js';
import { parseAgentConfig } from './domain/models/config.js';
...
export function loadConfiguration(configFile: string): SystemConfig {
  ...
  const verbose = false; // Could be added to config
...
export function createConfigFile(configPath: string, overwrite: boolean = false): void {
